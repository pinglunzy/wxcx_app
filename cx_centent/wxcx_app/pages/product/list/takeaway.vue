<template>
  <view class="container" :data-theme="theme()" :class="theme() || ''">
    <view class="main">
      <view class="nav">
        <view class="header">
          <view class="left" @click="selectShop('/pages/shop/shop_list')">
          ...
          <view class="right">
            <template v-for="(item, index) in delivery_set" :key="item">
              <view
                class="takeout"
                v-if="item == '10'"
                :class="orderType == 'takeout' ? 'active' : ''"
                @tap="takout"
              >
                <text>配送</text>
              </view>
                    ...
                    >
                      ...
                      <view class="right">
                        ...
                        <view class="price_and_action">
                          <view>
                            <text style="color: #ff5800; font-size: 22rpx"
                              >￥</text
                            >
                          ...
                          <view class="btn-group" v-else>
                            ...
                            >
                              <view
                                class="icon icon-jian iconfont iconsami-select"
                              >
                              </view>
                            </button>
      ...
      <view class="cart-box" v-if="cart_total_num > 0">
        ...
        <view class="price">
          <view>
            <text class="f22">￥</text>
            {{ total_price }}
            <text class="gray9 f22 text-d-line fn ml10"
              >￥{{ line_price }}</text
            >
          </view>
        ...
        <button
          class="pay-btn"
          @tap="toPay"
          v-if="min_money_diff <= 0 || orderType != 'takeout'"
        >
          去结算
        </button>
        <button
          ...
          disabled
          v-if="
            min_money_diff > 0 && total_price == 0 && orderType == 'takeout'
          "
        >
          {{ "￥" + min_money + "起送" }}
        </button>
    ...
    <popup-layer
      ...
      class="cart-popup"
      v-if="cart_total_num > 0"
    >
      <template #content>
        <view class="cart-popup pr">
          <view
            class="d-b-c top-title"
            v-if="reduce && (reduce.now || reduce.next)"
          >
            <view class="mj d-c-c">
                ...
                <view class="item" v-if="item.product_num > 0">
                  <view class="d-s-c">
                    ...
                    <view class="left">
                      ...
                      <view class="center">
                        ...
                        <text class="f24 gray9" v-if="bag_type != 1"
                          >(包装费：￥{{ item.bag_price }})</text
                        >
                      </view>
                    </view>
                  </view>
                  <view class="right">
                    <button
                      plain
                      size="min"
                      class="btn theme-borderbtn"
                      hover-class="none"
                      @tap="cartReduce(item)"
                    >
                      <view
                        class="iconfont icon icon-jian iconsami-select"
                      ></view>
                    </button>
                    ...
                    >
                      <view
                        class="iconfont icon icon-jia iconadd-select"
                      ></view>
                    </button>
                  </view>
                </view>
...
export default {
  ...
  methods: {
    ...
    getCategory() {
      ...
      self._get(
        ...
        function (res) {
          if (
            self.getUserId() &&
            res.data.address_id == 0 &&
            self.orderType == "takeout"
          ) {
            self.showError("未选择收货地址，请设置收货地址", function () {
              self.gotoPage(
                "/pages/user/address/storeaddress?shop_supplier_id=" +
    ...
    handleMenuTap(id) {
      ...
      this.$nextTick(
        () =>
          (this.cateScrollTop = this.goods_list.find(
    ...
    closeGoodDetailModal(num, res) {
      ...
      if (num) {
        this.$set(
          this.good,
          "cart_num",
          this.good.cart_num ? this.good.cart_num + num : num
        );
        this.reCart(res);
      }
      this.category = {};
    ...
    isBusiness() {
      ...
      if (this.orderType == "takeout") {
        if (
          this.supplier.delivery_time[0] <= now &&
          this.supplier.delivery_time[1] >= now
        ) {
          this.businessOpen = 0;
        } else {
          this.businessOpen = 1;
        ...
        if (
          this.supplier.pick_time[0] <= now &&
          this.supplier.pick_time[1] >= now
        ) {
          this.businessOpen = 0;
        } else {
          this.businessOpen = 1;
    ...
    toPay() {
      ...
      self._get(
        ...
        function (res) {
          ...
          uni.navigateTo({
            url:
              "/pages/order/confirm-order?order_type=cart&cart_ids=" +
              arrIds.join(",") +
              "&delivery=" +
              delivery +
              "&shop_supplier_id=" +
