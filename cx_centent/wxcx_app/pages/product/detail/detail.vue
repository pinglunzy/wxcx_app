        ...
        <view class="properties">
          <view class="property" v-if="detail.spec_type == 20">
            ...
            <view class="values">
              <view
                class="value"
                @click="selecedtSpec(value, key)"
                v-for="(value, key) in detail.sku"
                :key="key"
          ...
          <view
            class="property"
            v-if="detail.product_attr.length > 0"
            v-for="(item, index) in detail.product_attr"
            :key="index"
          >
            <view class="title">
...
export default {
  ...
  computed: {
    price: function () {
      if (this.discount_price) {
        return (
          (this.discount_price * 1 + this.product_price * 1) *
          this.form.show_sku.sum
        ).toFixed(2);
      } else {
        return (
          (this.feed_price * 1 + this.product_price * 1) *
          this.form.show_sku.sum
        ).toFixed(2);
      }
    },
    lineprice: function () {
      return (
        (this.feed_price * 1 + this.product_lineprice * 1) *
        this.form.show_sku.sum
      ).toFixed(2);
    },
    /*判断增加数量*/
    isadd: function () {
      return (
        this.form.show_sku.sum >= this.stock ||
        this.form.show_sku.sum >= this.form.detail.limit_num
      );
    },
  ...
  methods: {
    ...
    selectFeed(item, index) {
      let self = this;
      if (
        self.form.show_sku.feed[index] ||
        self.form.show_sku.feed[index] === 0
      ) {
    ...
    confirmFunc() {
      ...
      if (
        this.form.show_sku.product_sku_id[0] == null &&
        this.form.detail.spec_type == 20
      ) {
        uni.showToast({
          title: "请选择规格",
          icon: "none",
